...
void Camera::finishMovement(double deltaTime){
    ...
    double totalDistMod = pow(sin(M_PI/2*anim/maxAnimTime), 2);
    double nextDistX = diffX*totalDistMod-progressX;
    x += nextDistX;
    progressX += nextDistX;
    double nextDisY = diffY*totalDistMod-progressY;
    y += nextDistY;
    progressY += nextDistY;
